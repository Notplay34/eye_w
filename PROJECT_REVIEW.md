...
| Автотесты | ❌ |
| Миграции БД (версионированные) | ❌ (частично через startup) |
| Секреты не в коде (кроме дефолтного суперпользователя) | ⚠️ |
| Нет мёртвого кода (django_backend) | ⚠️ проверить |
...
Проект в рабочем состоянии, подходит для внутренней эксплуатации и уже развёрнут. Главные риски: отсутствие тестов (регрессии), захардкоженный суперпользователь (безопасность), схема БД без формальных миграций (эволюция без поломок). Рекомендуемый следующий шаг: выполнить пункты раздела «Сейчас» (очистка репо, суперпользователь из env, уточнение документации деплоя), затем добавить минимальные тесты и зафиксировать правила изменения схемы БД.
