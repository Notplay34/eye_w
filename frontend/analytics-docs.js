(function () {
  ...
  var kpiEl = document.getElementById('kpiDocsContent');
  var dynEl = document.getElementById('dynamicsDocsContent');
  var empEl = document.getElementById('employeesDocsContent');
  ...
  function getSummaryPeriod() {
    var sel = document.getElementById('summaryPeriodDocs');
  ...
  function getDynamicsGroup() {
    var sel = document.getElementById('dynamicsGroupDocs');
    var v = sel && sel.value ? sel.value : 'day';
    if (v !== 'day' && v !== 'week' && v !== 'month') return 'day';
    return v;
  }

  async function loadDynamicsDocs() {
    if (!dynEl) return;
    ...
    try {
      ...
      var rows = points.map(function (p) {
        return (
          ...
          '<td>' + formatMoney(p.income_pavilion1) + '</td>' +
      ...
      dynEl.innerHTML =
        ...
        '<thead><tr><th>Период</th><th>Выручка, всего</th><th>Доход Документы</th><th>Чистый доход</th></tr></thead>' +
        '<tbody>' +
        rows.join('') +
        '</tbody></table></div>';
  ...
  function getEmpPeriodDocs() {
    var sel = document.getElementById('empPeriodDocs');
    var v = sel && sel.value ? sel.value : 'day';
    if (v !== 'day' && v !== 'week' && v !== 'month') return 'day';
    return v;
  }

  async function loadEmployeesDocs() {
    if (!empEl) return;
    setLoading(empEl, true);
    var period = getEmpPeriodDocs();
    try {
      var r = await fetchApi(
        API_BASE + '/analytics/employees?period=' + encodeURIComponent(period) + '&kind=docs'
      );
      if (!r.ok) throw new Error(r.statusText);
      var data = await r.json();
      ...
      // По умолчанию сортируем по сумме, по убыванию
  ...
  function bindControls() {
    var summarySel = document.getElementById('summaryPeriodDocs');
    if (summarySel) summarySel.addEventListener('change', loadSummaryDocs);

    var dynSel = document.getElementById('dynamicsGroupDocs');
    if (dynSel) dynSel.addEventListener('change', loadDynamicsDocs);

    var empSel = document.getElementById('empPeriodDocs');
    if (empSel) empSel.addEventListener('change', loadEmployeesDocs);
  }
