...
**Оператор работает через веб-интерфейс по рабочей ссылке** (без Telegram).

**Функциональные требования:**
- склад заготовок номеров (учёт остатков, резервирование, списание)
- касса павильона (приём доплат за номера)
- база не выданных номеров (документы в статусе «передано в производство», ожидающие выдачи)
- просмотр заказа: тип номера, статус оплаты, сумма
- отметки: «Изготовлен», «Доплата получена», «Проблема»
...
### 4.3 Веб-интерфейс павильона 2
- **Рабочая ссылка** для оператора изготовления номеров
- Доступ: склад, касса павильона, база не выданных номеров
- Аналитика для владельца: сводки за день/месяц, данные по сотрудникам — через веб-админку
...
| `cash_rows` | Касса павильона 1 (ФИО, заявление, госпошлина, ДКП, страховка, номера, итого) |
| `plate_cash_rows` | Касса павильона 2 (номера) — фамилия и сумма (включая отрицательные суммы) |
| `plate_payouts` | Реестр сумм за номера, которые нужно передать из кассы документов в кассу номеров (по заказам со статусом «Выдано») |
...
## 11. Веб-интерфейс павильона 2 (изготовление номеров)

Оператор павильона 2:

- входит по рабочей ссылке (логин/пароль, JWT)
- видит базу не выданных номеров (документы, переданные в производство)
- работает со складом (остатки, резервирование, списание)
- принимает доплаты через кассу павильона
- меняет статус заказа в рамках разрешённого workflow

Все данные — в PostgreSQL, веб-интерфейс не хранит состояние локально.
...
Веб-интерфейс и админка не хранят состояние локально.
...
**Шаблоны docx** хранятся в папке `templates/` в корне проекта (перенесены из dist при его удалении).
